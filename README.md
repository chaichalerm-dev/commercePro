...
| คุณภาพโค้ด | PHPUnit (ผ่าน 99 เคส) · PHPStan level 6 · Laravel Pint (มาตรฐาน PSR-12) |
...
- ⚡ **ประสิทธิภาพ:** eager loading ป้องกันปัญหา N+1, แบ่งหน้า (pagination), ทำ index ฐานข้อมูล, โหลดรูปภาพแบบ lazy, ลดจำนวน query ที่ซ้ำซ้อนในทุกหน้า, รูปที่อัปโหลดถูก resize + แปลงเป็น WebP อัตโนมัติ พร้อม cache header อายุ 1 ปีสำหรับไฟล์ที่อัปโหลด (ดู `App\Support\ImageOptimizer`)
...
> ⚠️ ต้องเป็นพอร์ต **6543** (transaction-mode pooler) ไม่ใช่ 5432 (session-mode) — session-mode จำกัด connection รวมทั้งโปรเจกต์แค่ราว 15 connection และแอปนี้ใช้ persistent PDO connection ต่อ worker หนึ่งค้างไว้ตลอด ทำให้ connection เต็มไวมากถ้าใช้ผิดพอร์ต (อาการ: `SQLSTATE[08006] ... max clients reached`)
...
ทุกบัญชีใช้รหัสผ่าน `password` เหมือนกัน กล่องข้อความบัญชีทดลองในหน้า login **ปิดอยู่โดย default** และจะไม่แสดงบน production ไม่ว่าจะตั้งค่าอย่างไรก็ตาม (ป้องกันไม่ให้รหัสผ่านบัญชี Owner หลุดสู่สาธารณะ) — เปิดใช้งานสำหรับ local/staging ได้จาก **Admin → ตั้งค่าเว็บไซต์**
...
| — | Security audit: ปิดการแสดงรหัสผ่านบัญชีทดลองบน production, กรอง script ออกจากไฟล์ SVG ที่อัปโหลด | ✅ |
| — | รวม migration ทั้งหมดเป็นไฟล์เดียว เพื่อให้ติดตั้งโปรเจกต์ใหม่ง่ายขึ้น | ✅ |
| — | เพิ่มระบบ resize/บีบอัดรูปภาพเป็น WebP อัตโนมัติตอนอัปโหลด พร้อม cache header ระยะยาวและ response compression | ✅ |
