...
class OrderService
{
    ...
    public function placeFromCart(User $user, Address $address, ?string $couponCode = null): Order
    {
        ...
        if ($items->isEmpty()) {
            throw ValidationException::withMessages(['cart' => 'ตะกร้าสินค้าของคุณว่างเปล่า']);
        }
        ...
        $order = DB::transaction(function () use ($user, $address, $couponCode, $items): Order {
            ...
            foreach ($items as $item) {
                ...
                if ($product->status !== ProductStatus::Active) {
                    throw ValidationException::withMessages(['cart' => "สินค้า \"{$product->name}\" ไม่พร้อมจำหน่ายแล้ว กรุณานำออกจากตะกร้า"]);
                }
        ...
        Mail::to($user)->queue(new OrderPlacedMail($order));
    ...
    public function cancel(Order $order): Order
    {
        if (! $order->isCancellable()) {
            throw ValidationException::withMessages(['status' => 'คำสั่งซื้อนี้ไม่สามารถยกเลิกได้แล้ว']);
        }
    ...
    public function transition(Order $order, OrderStatus $to): Order
    {
        if (! in_array($to, $order->status->transitions(), true)) {
            throw ValidationException::withMessages([
                'status' => "เปลี่ยนสถานะจาก {$order->status->label()} เป็น {$to->label()} ไม่ได้",
            ]);
        }
    ...
    protected function resolveCoupon(?string $code, float $subtotal): ?Coupon
    {
        ...
        if ($coupon === null || ! $coupon->isRedeemable($subtotal)) {
            throw ValidationException::withMessages(['coupon' => 'คูปองนี้ใช้ไม่ได้กับคำสั่งซื้อของคุณ']);
        }
