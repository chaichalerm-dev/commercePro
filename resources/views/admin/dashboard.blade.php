<x-admin-layout :title="__('admin/dashboard.title')">
    {{-- Stat cards --}}
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <x-admin.stat-card label="{{ __('admin/dashboard.cards.revenue') }}" :value="money($cards['revenue'])" color="primary"
            icon="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        <x-admin.stat-card label="{{ __('admin/dashboard.cards.orders') }}" :value="number_format($cards['orders'])" color="blue"
        ...
        <x-admin.stat-card label="{{ __('admin/dashboard.cards.pending_orders') }}" :value="number_format($cards['pending_orders'])" color="amber"
            icon="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
        <x-admin.stat-card label="{{ __('admin/dashboard.cards.products') }}" :value="number_format($cards['products'])" color="violet"
            icon="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />
        <x-admin.stat-card label="{{ __('admin/dashboard.cards.customers') }}" :value="number_format($cards['customers'])" color="emerald"
    ...
    <div class="mt-6 grid gap-4 lg:grid-cols-3">
        <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm lg:col-span-2">
            <h2 class="font-semibold text-gray-900">{{ __('admin/dashboard.revenue_chart_title') }}</h2>
            <div class="mt-4 h-72">
                <canvas id="revenueChart"></canvas>
            </div>
        </div>
        <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            <h2 class="font-semibold text-gray-900">{{ __('admin/dashboard.status_chart_title') }}</h2>
    ...
    <div class="mt-6 grid gap-4 lg:grid-cols-3">
        <div class="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm lg:col-span-2">
            <div class="flex items-center justify-between border-b border-gray-100 p-5">
                <h2 class="font-semibold text-gray-900">{{ __('admin/dashboard.recent_orders.title') }}</h2>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm">
                    <thead class="bg-gray-50 text-xs uppercase text-gray-400">
                        <tr>
                            <th class="px-5 py-3 font-medium">{{ __('admin/dashboard.table.order_number') }}</th>
                            <th class="px-5 py-3 font-medium">{{ __('admin/dashboard.table.customer') }}</th>
                            <th class="px-5 py-3 font-medium">{{ __('admin/dashboard.table.total') }}</th>
                            <th class="px-5 py-3 font-medium">{{ __('admin/dashboard.table.status') }}</th>
                            <th class="px-5 py-3 font-medium">{{ __('admin/dashboard.table.payment') }}</th>
                            <th class="px-5 py-3 font-medium">{{ __('admin/dashboard.table.date') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50">
                        ...
                        @empty
                            <tr><td colspan="6" class="px-5 py-8 text-center text-gray-400">{{ __('admin/dashboard.recent_orders.empty') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        {{-- Top products --}}
        <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            <h2 class="font-semibold text-gray-900">{{ __('admin/dashboard.top_products.title') }}</h2>
                ...
                @forelse ($topProducts as $item)
                    <li class="flex items-center gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-xs text-gray-400">{{ __('admin/dashboard.top_products.sold_qty', ['qty' => number_format((int) $item->qty_sold)]) }}</p>
                        </div>
                        <span class="text-sm font-semibold text-gray-700">{{ money((float) $item->revenue) }}</span>
                    </li>
                @empty
                    <li class="text-sm text-gray-400">{{ __('admin/dashboard.top_products.empty') }}</li>
                @endforelse
            </ol>
        </div>
    </div>
    ...
    <div class="mt-6 grid gap-4 lg:grid-cols-2">
        <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            <h2 class="flex items-center gap-2 font-semibold text-gray-900">
                {{ __('admin/dashboard.low_stock.title') }}
            ...
            <ul class="mt-4 space-y-3">
                ...
                @empty
                    <li class="text-sm text-gray-400">{{ __('admin/dashboard.low_stock.empty') }}</li>
                @endforelse
            </ul>
        </div>

        <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            <h2 class="font-semibold text-gray-900">{{ __('admin/dashboard.latest_customers.title') }}</h2>
            <ul class="mt-4 space-y-3">
                ...
                @empty
                    <li class="text-sm text-gray-400">{{ __('admin/dashboard.latest_customers.empty') }}</li>
                @endforelse
            </ul>
        </div>
    </div>
