<x-admin-layout :title="__('admin/banners.title')">
    <div class="flex items-center justify-between">
        <p class="text-sm text-gray-500">{{ __('admin/banners.total_count', ['count' => $banners->total()]) }}</p>
            ...
            <div class="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
                ...
                <div class="p-4">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="ml-auto text-xs text-gray-400">{{ __('admin/banners.sort_order_label', ['n' => $banner->sort_order]) }}</span>
                    </div>
                    ...
                    <div class="mt-3 flex gap-1.5">
                        <a href="{{ route('admin.banners.edit', $banner) }}" class="rounded-lg bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-600 transition hover:bg-blue-100">{{ __('admin/banners.table.edit') }}</a>
                        <form method="POST" action="{{ route('admin.banners.destroy', $banner) }}" onsubmit="return confirmSubmit(event, '{{ __('admin/banners.confirm.delete') }}')">
                            @csrf @method('DELETE')
                            <button type="submit" class="rounded-lg bg-red-50 px-3 py-1.5 text-xs font-medium text-red-500 transition hover:bg-red-100">{{ __('admin/banners.table.delete') }}</button>
                        </form>
                    </div>
                </div>
            </div>
        @empty
            <div class="col-span-full rounded-2xl border border-dashed border-gray-200 bg-white py-12 text-center text-gray-400">{{ __('admin/banners.empty_state') }}</div>
        @endforelse
    </div>
