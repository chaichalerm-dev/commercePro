...
<form method="POST"
    ...
    <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
        <div class="grid gap-4 sm:grid-cols-2">
            <div class="sm:col-span-2">
                <label for="title" class="block text-sm font-medium text-gray-700">หัวข้อ <span class="text-red-500">*</span></label>
            ...
            <div class="sm:col-span-2">
                <label for="subtitle" class="block text-sm font-medium text-gray-700">คำโปรย</label>
                <input id="subtitle" type="text" name="subtitle" value="{{ old('subtitle', $banner?->subtitle) }}"
                       class="mt-1.5 w-full rounded-xl border-gray-200 text-sm focus:border-primary-400 focus:ring-primary-400">
            </div>

            <div>
                <label for="link" class="block text-sm font-medium text-gray-700">ลิงก์ปลายทาง</label>
            ...
            <div class="flex items-end pb-2">
                <label class="flex cursor-pointer items-center gap-2">
                    ...
                    <span class="text-sm text-gray-700">แสดงแบนเนอร์นี้</span>
                </label>
            </div>
        </div>

        <div class="mt-4" x-data="{ preview: {{ Js::from($banner?->image_url) }} }">
            <label class="block text-sm font-medium text-gray-700">รูปแบนเนอร์ {{ $banner ? '' : '*' }} <span class="text-xs text-gray-400">(แนะนำ 1600×600 สำหรับ Hero)</span></label>
    ...
    <div class="flex gap-2">
        ...
        <a href="{{ route('admin.banners.index') }}" class="rounded-xl border border-gray-200 bg-white px-5 py-2.5 text-sm text-gray-600 transition hover:bg-gray-50">ยกเลิก</a>
    </div>
</form>
