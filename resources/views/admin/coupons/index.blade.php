<x-admin-layout title="คูปองส่วนลด">
    <div class="flex items-center justify-between">
        <p class="text-sm text-gray-500">ทั้งหมด {{ $coupons->total() }} คูปอง</p>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                <thead class="bg-gray-50 text-xs uppercase text-gray-400">
                    <tr>
                        <th class="px-5 py-3 font-medium">โค้ด</th>
                        <th class="px-5 py-3 font-medium">ส่วนลด</th>
                        <th class="px-5 py-3 font-medium">ขั้นต่ำ</th>
                        <th class="px-5 py-3 font-medium">ใช้ไปแล้ว</th>
                        <th class="px-5 py-3 font-medium">หมดอายุ</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse ($coupons as $coupon)
                        <tr class="hover:bg-gray-50/60">
                            ...
                            <td class="px-5 py-3 text-gray-500">{{ $coupon->expires_at?->format('d/m/Y') ?? 'ไม่มีกำหนด' }}</td>
                            <td class="px-5 py-3">
                                ...
                                <span class="rounded-full px-2.5 py-1 text-xs font-medium {{ $usable ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-500' }}">
                                    {{ $usable ? 'ใช้งานได้' : ($coupon->is_active ? 'หมดอายุ/เต็ม' : 'ปิดใช้งาน') }}
                                </span>
                            </td>
                            <td class="px-5 py-3">
                                <div class="flex items-center justify-end gap-1.5">
                                    <a href="{{ route('admin.coupons.edit', $coupon) }}" class="rounded-lg bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-600 transition hover:bg-blue-100">แก้ไข</a>
                                    <form method="POST" action="{{ route('admin.coupons.destroy', $coupon) }}" onsubmit="return confirm('ลบคูปองนี้?')">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="7" class="px-5 py-10 text-center text-gray-400">ยังไม่มีคูปอง</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
