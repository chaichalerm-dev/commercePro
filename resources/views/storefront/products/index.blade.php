@php
    $heading = isset($category)
        ? $category->name
        : (filled($filters['q']) ? 'ผลการค้นหา "'.$filters['q'].'"' : ($filters['on_sale'] ? 'โปรโมชั่น' : 'สินค้าทั้งหมด'));
    $formAction = isset($category) ? route('categories.show', $category->slug) : route('products.index');
@endphp

<x-storefront-layout :title="$heading" :description="isset($category) ? 'เลือกซื้อสินค้าในหมวด '.$category->name : 'ค้นหาและเลือกซื้อสินค้าคุณภาพราคาพิเศษ'">
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <x-breadcrumb :items="isset($category) ? ['สินค้าทั้งหมด' => route('products.index'), $category->name => null] : [$heading => null]" />
            ...
            <aside x-data="{ open: false }" class="lg:sticky lg:top-32 lg:self-start">
                ...
                <form action="{{ $formAction }}" method="GET" x-show="open || window.innerWidth >= 1024" x-cloak
                    ...
                    @unless (isset($category))
                        <div>
                            <h3 class="text-sm font-semibold text-gray-800">หมวดหมู่</h3>
                    ...
                    <div>
                        <h3 class="text-sm font-semibold text-gray-800">ช่วงราคา (บาท)</h3>
                        <div class="mt-2 flex items-center gap-2">
                            <input type="number" name="min_price" value="{{ $filters['min_price'] }}" min="0" placeholder="ต่ำสุด"
                                   class="w-full rounded-lg border-gray-200 text-sm focus:border-primary-400 focus:ring-primary-400">
                            <span class="text-gray-400">–</span>
                            <input type="number" name="max_price" value="{{ $filters['max_price'] }}" min="0" placeholder="สูงสุด"
                                   class="w-full rounded-lg border-gray-200 text-sm focus:border-primary-400 focus:ring-primary-400">
                        </div>
                    </div>
                    ...
                    <div class="flex gap-2">
                        <button type="submit" class="flex-1 rounded-xl bg-primary-500 py-2.5 text-sm font-semibold text-white transition hover:bg-primary-600">ใช้ตัวกรอง</button>
                        <a href="{{ $formAction }}" class="rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-500 transition hover:bg-gray-50">ล้าง</a>
                    </div>
                </form>
            </aside>
            ...
            <div>
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-0.5 text-sm text-gray-400">พบ {{ number_format($products->total()) }} รายการ</p>
                    </div>
                    <form action="{{ $formAction }}" method="GET" class="flex items-center gap-2">
                        ...
                        @endif
                        <label for="sort" class="text-sm text-gray-500">เรียงตาม</label>
                        <select id="sort" name="sort" onchange="this.form.submit()"
                                class="rounded-lg border-gray-200 text-sm focus:border-primary-400 focus:ring-primary-400">
                            <option value="latest" @selected($filters['sort'] === 'latest')>ใหม่ล่าสุด</option>
                            <option value="popular" @selected($filters['sort'] === 'popular')>ยอดนิยม</option>
                            <option value="price_asc" @selected($filters['sort'] === 'price_asc')>ราคา: ต่ำ → สูง</option>
                            <option value="price_desc" @selected($filters['sort'] === 'price_desc')>ราคา: สูง → ต่ำ</option>
                        </select>
                    </form>
                </div>
                ...
                @if ($products->isEmpty())
                    <div class="mt-10 flex flex-col items-center rounded-2xl border border-dashed border-gray-200 bg-white py-16 text-center">
                        ...
                        <p class="mt-4 font-medium text-gray-700">ไม่พบสินค้าที่ตรงกับเงื่อนไข</p>
                        <p class="mt-1 text-sm text-gray-400">ลองเปลี่ยนคำค้นหาหรือล้างตัวกรองดูอีกครั้ง</p>
                        <a href="{{ route('products.index') }}" class="mt-5 rounded-xl bg-primary-500 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-primary-600">ดูสินค้าทั้งหมด</a>
                    </div>
                @else
