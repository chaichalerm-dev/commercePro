<x-storefront-layout title="คำสั่งซื้อของฉัน">
    <div class="mx-auto max-w-4xl px-4 py-6 sm:px-6 lg:px-8">
        <x-breadcrumb :items="['คำสั่งซื้อของฉัน' => null]" />
        <h1 class="mt-4 text-2xl font-bold text-gray-900">คำสั่งซื้อของฉัน</h1>
        ...
        @if ($orders->isEmpty())
            <div class="mt-8 flex flex-col items-center rounded-2xl border border-dashed border-gray-200 bg-white py-16 text-center">
                ...
                <p class="mt-4 font-medium text-gray-700">คุณยังไม่มีคำสั่งซื้อ</p>
                <a href="{{ route('products.index') }}" class="mt-5 rounded-xl bg-primary-500 px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-primary-600">เริ่มช้อปเลย</a>
            </div>
        @else
                    ...
                    <a href="{{ route('orders.show', $order) }}"
                       class="flex flex-wrap items-center gap-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition hover:border-primary-200 hover:shadow-md">
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="mt-0.5 text-xs text-gray-400">{{ $order->created_at->format('d/m/Y H:i') }} · {{ $order->items_count }} รายการ</p>
                        </div>
