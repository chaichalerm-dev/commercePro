<x-storefront-layout>
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            ...
            @if ($heroBanners->isNotEmpty())
                <div class="relative overflow-hidden rounded-2xl shadow-sm"
                    ...
                    @foreach ($heroBanners as $banner)
                        <div x-show="current === {{ $loop->index }}"
                            ...
                            <div class="absolute inset-0 flex flex-col justify-center bg-gradient-to-r from-black/50 via-black/20 to-transparent p-8 sm:p-12">
                                <span class="mb-3 w-fit rounded-full bg-primary-500 px-3 py-1 text-xs font-semibold text-white">โปรโมชั่นพิเศษ</span>
                    ...
                    <div class="absolute bottom-4 left-1/2 flex -translate-x-1/2 gap-1.5">
                        @foreach ($heroBanners as $banner)
                            <button @click="current = {{ $loop->index }}" aria-label="สไลด์ {{ $loop->iteration }}"
        ...
        <div class="mt-6 grid grid-cols-2 divide-gray-100 rounded-2xl border border-gray-100 bg-white shadow-sm sm:grid-cols-4 sm:divide-x">
            @foreach ([
                ['title' => 'จัดส่งฟรี', 'subtitle' => 'เมื่อช้อปครบ '.number_format($freeShippingMin).' บาท', 'icon' => 'M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 00-3.213-9.193 2.056 2.056 0 00-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 00-10.026 0 1.106 1.106 0 00-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12'],
                ['title' => 'คืนสินค้าฟรี', 'subtitle' => 'ภายใน 7 วัน', 'icon' => 'M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99'],
                ['title' => 'ชำระเงินปลอดภัย', 'subtitle' => 'ระบบ SSL 100%', 'icon' => 'M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z'],
                ['title' => 'บริการลูกค้า 24 ชม.', 'subtitle' => 'พร้อมให้บริการ', 'icon' => 'M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z'],
            ] as $feature)
        ...
        <section class="mt-10" aria-labelledby="featured-heading">
            <div class="flex items-center justify-between">
                <h2 id="featured-heading" class="text-xl font-bold text-gray-900">สินค้าแนะนำ</h2>
                <a href="{{ route('products.index') }}" class="text-sm font-medium text-primary-600 hover:text-primary-700">ดูทั้งหมด →</a>
            </div>
        ...
        <section class="mt-10" aria-labelledby="latest-heading">
            <div class="flex items-center justify-between">
                <h2 id="latest-heading" class="text-xl font-bold text-gray-900">สินค้าใหม่ล่าสุด</h2>
                <a href="{{ route('products.index', ['sort' => 'latest']) }}" class="text-sm font-medium text-primary-600 hover:text-primary-700">ดูทั้งหมด →</a>
            </div>
