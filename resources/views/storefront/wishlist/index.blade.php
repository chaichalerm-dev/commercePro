<x-storefront-layout :title="__('storefront/wishlist.title')">
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <x-breadcrumb :items="[__('storefront/wishlist.breadcrumb') => null]" />
        <h1 class="mt-4 text-2xl font-bold text-gray-900">{{ __('storefront/wishlist.title') }}</h1>
        ...
        @if ($wishlists->isEmpty())
            <div class="mt-8 flex flex-col items-center rounded-2xl border border-dashed border-gray-200 bg-white py-16 text-center">
                ...
                <p class="mt-4 font-medium text-gray-700">{{ __('storefront/wishlist.empty.title') }}</p>
                <a href="{{ route('products.index') }}" class="mt-5 rounded-xl bg-primary-500 px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-primary-600">{{ __('storefront/wishlist.empty.cta') }}</a>
            </div>
        @else
                ...
                @foreach ($wishlists as $wishlist)
                    <div class="relative">
                        ...
                        <form method="POST" action="{{ route('wishlist.toggle') }}" class="absolute right-3 top-3 z-10">
                            ...
                            <button type="submit" title="{{ __('storefront/wishlist.remove_title') }}"
