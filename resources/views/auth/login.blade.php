<x-guest-layout>
    <div x-data="{ tab: '{{ old('portal', 'user') }}' }">
        <h1 class="text-center text-lg font-bold text-gray-900">{{ __('auth_pages.login.title') }}</h1>

        {{-- User / Admin tabs: the selected tab is submitted as the "portal"
             field and enforced server-side in LoginRequest::authenticate(),
             so an admin tab login is rejected for non-admin credentials
             and vice versa. --}}
        ...
        <form method="POST" action="{{ route('login') }}" class="mt-5">
            @csrf
            <input type="hidden" name="portal" :value="tab">

            <div>
                <label for="email" class="block text-sm font-medium text-gray-700">{{ __('auth_pages.login.email') }}</label>
            ...
            <div class="mt-4" x-data="{ show: false }">
                <label for="password" class="block text-sm font-medium text-gray-700">{{ __('auth_pages.login.password') }}</label>
            ...
            <div class="mt-4 flex items-center justify-between">
                <label for="remember_me" class="inline-flex items-center">
                    ...
                    <span class="ms-2 text-sm text-gray-600">{{ __('auth_pages.login.remember_me') }}</span>
                </label>
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" class="text-sm text-primary-600 hover:text-primary-700 hover:underline">{{ __('auth_pages.login.forgot_password') }}</a>
                @endif
            </div>
        ...
        <div class="mt-5 flex items-center gap-3 text-xs text-gray-400">
            <span class="h-px flex-1 bg-gray-100"></span>{{ __('auth_pages.login.or') }}<span class="h-px flex-1 bg-gray-100"></span>
        </div>
