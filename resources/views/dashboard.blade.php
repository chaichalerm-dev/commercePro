<x-storefront-layout title="บัญชีของฉัน">
    <div class="mx-auto max-w-4xl px-4 py-6 sm:px-6 lg:px-8">
        <h1 class="text-2xl font-bold text-gray-900">สวัสดี, {{ auth()->user()->name }} 👋</h1>
        <p class="mt-1 text-sm text-gray-500">จัดการบัญชีและติดตามคำสั่งซื้อของคุณได้จากที่นี่</p>
        ...
        <div class="mt-6 grid gap-4 sm:grid-cols-3">
            <a href="{{ route('orders.index') }}" class="group rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition hover:border-primary-200 hover:shadow-md">
                ...
                <p class="mt-3 font-semibold text-gray-800 group-hover:text-primary-600">คำสั่งซื้อของฉัน</p>
                <p class="mt-0.5 text-xs text-gray-400">{{ auth()->user()->orders()->count() }} รายการ</p>
            </a>
            <a href="{{ route('wishlist.index') }}" class="group rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition hover:border-primary-200 hover:shadow-md">
                ...
                <p class="mt-3 font-semibold text-gray-800 group-hover:text-primary-600">รายการโปรด</p>
                <p class="mt-0.5 text-xs text-gray-400">{{ auth()->user()->wishlists()->count() }} รายการ</p>
            </a>
            <a href="{{ route('profile.edit') }}" class="group rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition hover:border-primary-200 hover:shadow-md">
                ...
                <p class="mt-3 font-semibold text-gray-800 group-hover:text-primary-600">แก้ไขโปรไฟล์</p>
        ...
        @if ($recentOrders->isNotEmpty())
            <section class="mt-8">
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-bold text-gray-900">คำสั่งซื้อล่าสุด</h2>
                    <a href="{{ route('orders.index') }}" class="text-sm font-medium text-primary-600 hover:underline">ดูทั้งหมด →</a>
                </div>
